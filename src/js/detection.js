...
export const drawRect = (
  ...
  if (
    detections.filter((d) => d.class == "pile").some((d) => d.bbox.width > 18)
  ) {
    canvas2.current.classList.add("danger");
  } else {
    canvas2.current.classList.remove("danger");
  }
  detections.forEach((prediction) => {
    ...
    ctx.fillStyle = color;
    if (text == "pile" && width > 13) {
      beep();
      ctx.strokeStyle = warning;
      ctx.fillStyle = warning;
    }

    if (text == "pile" && width > 18) {
